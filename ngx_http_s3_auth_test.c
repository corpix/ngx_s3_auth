...
static void host_header_ctor(void **state) {
  ...
  host = ngx_s3_auth__host_from_bucket(pool, &bucket);
...
static void hmac_sha256(void **state) {
  ...
  hash = ngx_s3_auth__sign_sha256_hex(pool, &text, &key);
...
static void canon_header_string(void **state) {
  ...
  retval = ngx_s3_auth__canonize_headers(pool, NULL, &bucket, &date, &hash, &endpoint);
...
static void canonical_qs_empty(void **state) {
  ...
  const ngx_str_t *canon_qs = ngx_s3_auth__canonize_query_string(pool, &request);
...
static void canonical_url_sans_qs(void **state) {
  ...
  const ngx_str_t *canon_url = ngx_s3_auth__canon_url(pool, &request);
...
static void canonical_request_sans_qs(void **state) {
  ...
  const ngx_str_t s3_date = ngx_string("20160221T063112Z");
  ...
  result = ngx_s3_auth__make_canonical_request(pool, &request, &bucket, &s3_date, &endpoint);
...
static void basic_get_signature(void **state) {
  ...
  struct S3SignedRequestDetails result = ngx_s3_auth__compute_signature(pool, &request,
